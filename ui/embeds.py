...
def zone_info_embed(zone_id: str, player_level: int, active_mission: dict = None) -> discord.Embed:
    ...
    # Mission display
    if active_mission and active_mission.get("mission_target_item_id"):
        target_item_id = active_mission["mission_target_item_id"]
        target_qty = active_mission["mission_target_qty"]
        item = ITEMS.get(target_item_id, {"name": target_item_id, "emoji": "✨"})
        mission_text = f"{target_qty}x {item.get('emoji', '✨')} **{item['name']}**"
    else:
        mission_text = "Start a mission to see your challenge!"
...
def zone_harvest_embed(zone_id: str, item_id: str, rarity: str, metadata_type: str = None, 
    ...
    embed = discord.Embed(
        ...
        color=RARITY_COLORS_ZONE.get(rarity, 0x95A5A6),
    )
